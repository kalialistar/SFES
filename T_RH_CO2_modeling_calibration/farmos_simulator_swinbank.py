...
from farmos_model_swinbank import run_integrated_TRH_CO2_model

# ─────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="온실 미기후 시뮬레이터 (Farmos 딸기)",
...
OBS_META = [
    ...
    ("배지함수율", "Substrate Water", "m³/m³"),
    ("배지온도",   "Substrate Temp",  "°C"),
]
...
def normalize_input_columns(df: pd.DataFrame) -> pd.DataFrame:
    alias_groups = {
        ...
        "배지함수율": ["배지함수율", "배지 함수율", "substrate moisture", "substrate water content", "media water content", "theta_sub"],
        "배지온도": ["배지온도", "배지 온도", "substrate temp", "substrate temperature", "media temp", "ts_sub"],
...
def calc_threshold_error_stats(pred_series, obs_series, threshold):
    ...
    if n == 0:
        return None, None, 0
    abs_err = np.abs(p - o)
    exceed_mask = abs_err > float(threshold)
    exceed_count = int(np.sum(exceed_mask))
    exceed_pct = float(exceed_count / n * 100.0)
    return exceed_pct, exceed_count, n




def add_series_fig(fig, df, xcol, ycol, title=None):
    if ycol in df.columns and df[ycol].notna().any():
        info = PARAM_INFO.get(ycol, (ycol, '', '#2d6a4f'))
        fig.add_trace(go.Scatter(
            x=df[xcol], y=df[ycol], mode="lines", name=info[0],
            line=dict(color=info[2], width=2),
        ))
        fig.update_layout(yaxis_title=info[1], title=title or info[0])
    return fig


def calc_metrics(pred_series, obs_series):
    """RMSE, MBE 계산 (NaN 행 제거 후 계산)"""
    pred_arr = np.array(pred_series, dtype=float)
    obs_arr  = np.array(obs_series,  dtype=float)
    mask = np.isfinite(pred_arr) & np.isfinite(obs_arr)
    p = pred_arr[mask]
...
def render_folium_farmos():
    """NIHHS map HTML"""
    try:
        import folium
        # NIHHS official address: 100, Nongsaengmyeong-ro, Iseo-myeon, Wanju-gun, Jeonbuk.
        # The marker uses an on-campus coordinate reported for an NIHHS field plot in Wanju.
        nihhs_lat = 35.825111
        nihhs_lon = 127.025889
        m = folium.Map(
            location=[nihhs_lat, nihhs_lon],
            zoom_start=15,
            tiles="OpenStreetMap",
            control_scale=True,
        )
        folium.Marker(
            location=[nihhs_lat, nihhs_lon],
            popup=folium.Popup("<b>국립원예특작과학원</b><br>전북특별자치도 완주군 이서면 농생명로 100", max_width=240),
            tooltip="📍 국립원예특작과학원",
            icon=folium.Icon(color="green", icon="leaf", prefix="fa"),
        ).add_to(m)
        folium.Circle(
            location=[nihhs_lat, nihhs_lon],
            radius=500, color="#52b788", fill=True,
            fill_color="#74c69d", fill_opacity=0.25,
        ).add_to(m)
...
with st.sidebar:
    ...
    st.markdown("**Farmos 딸기 온실 미기후 모델**")
    st.divider()
...
# ─── 메인 ───────────────────────────────────────────────────────────
st.markdown('<div class="main-title">🍓 온실 미기후 시뮬레이터 | Farmos Strawberry</div>', unsafe_allow_html=True)
        ...
        with st.spinner("⚙️ 시뮬레이션 계산 중..."):
            try:
                ...
                merge_cols = [
                    ...
                    "강수량", "감우", "배지함수율", "배지온도", "내부온도", "내부습도", "내부CO2", "외부CO2"
                ]
...
if result_df is None:
    ...
    4. <b>시점 분석</b>: 날짜·시각 → 예측값·실측값 카드 + ±12h 가로 3개 그래프<br>5. 입력 필수 컬럼에 <b>배지함수율</b>, <b>배지온도</b>가 포함됩니다.
    </div>""", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**필수 컬럼**")
        st.table(pd.DataFrame({
            "컬럼명":["날짜&시간","외부 온도","외부 습도","일사","배지함수율","배지온도"],
            "단위":["datetime","°C","%","W/m²","m³/m³ 또는 %","°C"],
            "설명":["1분~1시간 간격 가능","외기 온도","외기 상대습도","태양복사","배지 체적함수율","배지 온도"],
        }))
    with c2:
        st.markdown("**선택 컬럼**")
        st.table(pd.DataFrame({
...
with tab1:
    ...
    m_pairs = [
        ("Troom_C", "내부온도", "온도", "°C"),
        ("RHin_%",  "내부습도", "습도", "%"),
        ("Cin_ppm", "내부CO2",  "CO₂", "ppm"),
    ]
    metric_rows = []
    threshold_map = {"온도": 2.0, "습도": 7.0, "CO₂": 30.0}
    for pk, ok, lbl, unit in m_pairs:
        ...
        threshold = threshold_map.get(lbl)
        rmse = mbe = exceed_pct = None
        if pred_ok and obs_ok:
            rmse, mbe = calc_metrics(sub_df[pk].values, sub_df[ok].values)
            exceed_pct, _, _ = calc_threshold_error_stats(
                sub_df[pk].values, sub_df[ok].values, threshold
            )
        metric_rows.append({
            "항목": lbl,
            "단위": unit,
...
with tab2:
    ...
    st.markdown('<div class="section-header">🧩 선택 시각 기준 세부 예측 항목 추이</div>', unsafe_allow_html=True)
    detail_group_t2 = st.selectbox('시점 분석 항목 그룹', list(DISPLAY_GROUPS.keys()), key='t2_detail_group')
    detail_params_t2 = st.multiselect(
        '시점 분석 그래프 항목',
        options=[k for k in DISPLAY_GROUPS[detail_group_t2] if k in w_df.columns],
        default=[k for k in DISPLAY_GROUPS[detail_group_t2][:3] if k in w_df.columns],
        format_func=lambda k: f"{PARAM_INFO.get(k,(k,'',''))[0]} [{PARAM_INFO.get(k,(k,'',''))[1]}]",
        key='t2_detail_params'
    )
    if not w_df.empty and detail_params_t2:
        fig_t2_detail = go.Figure()
        units_t2 = []
        for k in detail_params_t2:
            info = PARAM_INFO.get(k, (k, '', '#2d6a4f'))
            fig_t2_detail.add_trace(go.Scatter(
                x=w_df[TIME_COL], y=w_df[k], mode='lines', name=info[0],
                line=dict(color=info[2], width=2),
            ))
            if info[1] and info[1] not in units_t2:
                units_t2.append(info[1])
        fig_t2_detail.add_vline(x=sel_dt, line_dash='dash', line_color='red', opacity=0.75)
        fig_t2_detail.update_layout(
            height=380, hovermode='x unified', plot_bgcolor='#f9fffe',
            legend=dict(orientation='h', y=1.12, x=0),
            margin=dict(l=50, r=20, t=50, b=40),
            xaxis=dict(title='Time', tickformat='%m/%d\n%H:%M'),
            yaxis=dict(title=' / '.join(units_t2) if units_t2 else ''),
        )
        st.plotly_chart(fig_t2_detail, use_container_width=True)
...
# ─────────────────────────────────────────────────────────────────────
st.divider()
st.caption("📌 모델: FvCB 광합성 (Medlyn 기공) + Penman-Monteith 증산 + 다층 커버 열전달 + Swinbank 하늘온도 | 온실: 45×24×7 m | 작물: 딸기")