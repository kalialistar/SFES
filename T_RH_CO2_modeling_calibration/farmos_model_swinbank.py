...
cropArea    = areaHouse / 6.0                      # The cultivation area is one-sixth(1/6) of the actual floor area

Chouse  = 150000.0  # Factor of Temp -> Heat Capacity
CWhouse = 40000.0   # Factor of Humid -> water Capacity

agh = 0.15
Tgroundi = 15.0 + 273.15
cloud_default = 0.5
deltaT_leaf_air = -2.0

LAYER_PROPS = {
    "PO필름": {"tau_ir": 0.35, "eps_lw": 0.70, "alpha_sw": 0.15, "k": 0.25, "t": 0.00015},
    "AL스크린": {"tau_ir": 0.10, "eps_lw": 0.10, "alpha_sw": 0.10, "k": 1.00, "t": 0.0005},
    "보온커튼": {"tau_ir": 0.05, "eps_lw": 0.90, "alpha_sw": 0.15,  "k": 0.06, "t": 0.003},
}

a_sw_can = 0.56
k_sw = 0.86   # Beer-Lambert extinction of strawberry
...
VCMAX25 = 106.59        # umol m-2 s-1
EA_VCMAX = 53.20e3      # J mol-1
DS_VCMAX = 0.637e3      # J mol-1 K-1
HD_VCMAX = 200.0e3      # J mol-1

JMAX25 = 203.55         # umol m-2 s-1
EA_JMAX = 23.74e3       # J mol-1
DS_JMAX = 0.639e3       # J mol-1 K-1
...
Cin_min_ppm = 200.0
Cin_max_ppm = 2000.0
VCeff_night = 0.04  # Factor of CO2 -> effective air-mixing volume coefficient
...
# Jarvis/Stanghellini minimum leaf stomatal resistance
RSMIN_LEAF = 130.0  # s m-1 (initial strawberry value; to be calibrated)

# Substrate (coir) evaporation parameters
# Coir (100%) van Genuchten parameters
coir_theta_S = 0.915   # saturated water content
coir_theta_R = 0.182   # residual water content
coir_alpha = 11.7  # 1/m  (0.117 cm^-1)
coir_n = 1.725
soil_depth = 1.0  # L in h = Nu*k/L (user set)
...
gap_F_S = 0.20   # film-screen gap thickness [m]
...
def h_soil_from_Nu_flatplate(T_air_K: float, u_int: float, L: float = soil_depth):
...
def vg_psi_from_theta(theta: float,
                      theta_s: float = coir_theta_S,
                      theta_r: float = coir_theta_R,
                      alpha_m_inv: float = coir_alpha,
                      n: float = coir_n) -> float:
    """van Genuchten inverse: theta -> matric head psi (m, negative)."""
    th = float(theta)
    th = float(np.clip(th, theta_r + 1e-9, theta_s - 1e-9))
    Se = (th - theta_r) / max(theta_s - theta_r, 1e-12)
    Se = float(np.clip(Se, 1e-9, 1.0 - 1e-9))
    m = 1.0 - 1.0 / float(n)
    psi_abs_m = (1.0 / max(float(alpha_m_inv), 1e-12)) * ((Se ** (-1.0 / m) - 1.0) ** (1.0 / float(n)))
    return -float(psi_abs_m)


def kelvin_RH_surface(psi_m: float, Ts_K: float) -> float:
    """Kelvin equation: matric head (m, negative) + Ts -> RH at substrate surface."""
    psi_pa = float(psi_m) * 1000.0 * ga  # rho_w*g*psi_m (rho_w≈1000 kg/m3)
    RHs = math.exp((psi_pa * 0.018) / (R_univ * max(float(Ts_K), 200.0)))
    return float(np.clip(RHs, 0.0, 1.0))
...
def boundary_layer_conductance_paper(u_canopy: float, dleaf: float = LD) -> float:
...
def solve_leaf_fvcb_no_medlyn(ppfd_leaf: float, Tleaf_K: float, Ca_ppm: float):
    """
    Leaf-scale FvCB biochemical assimilation without Medlyn stomatal model.
    Ci is approximated from ambient CO2 using a fixed ratio for biochemical evaluation only.
    Stomatal conductance for ET is handled separately by Jarvis/Stanghellini resistance.
    Returns leaf net/gross A, Rd, J, Vcmax, and proxy Cs/Cc values.
    """
...
def jarvis_stanghellini_leaf_resistance(rad_wm2: float, Tleaf_K: float, VPD_kPa: float, Ca_ppm: float,
                                        rs_min_leaf_s_m: float = RSMIN_LEAF) -> float:
    rad = max(float(rad_wm2), 0.0)
    Tleaf_C = float(Tleaf_K) - 273.15
    D = max(float(VPD_kPa), 0.0)
    C = max(float(Ca_ppm), 1.0)
    f_rad = (rad + 4.3) / max(rad + 0.54, 1e-9)
    f_temp = 1.0 + 0.023 * ((Tleaf_C - 24.5) ** 2)
    f_vpd = (1.0 + 4.3 * D * D) if D < 0.8 else 3.8
    f_co2 = (1.0 + 6.1e-7 * ((C - 200.0) ** 2)) if C < 1100.0 else 1.5
    return float(max(rs_min_leaf_s_m * f_rad * f_temp * f_vpd * f_co2, 1e-6))

def effective_lai_stanghellini(LAI_val: float) -> float:
    lai = max(float(LAI_val), 0.0)
    if lai <= 2.0:
        return float(max(lai, 1e-9))
    if lai < 4.0:
        return 2.0
    return float(max(0.5 * lai, 1e-9))

def stomatal_and_canopy_resistance_from_jarvis(rad_wm2: float, Tleaf_K: float, VPD_kPa: float, Ca_ppm: float,
                                               LAI_val: float):
    rs_leaf = jarvis_stanghellini_leaf_resistance(rad_wm2, Tleaf_K, VPD_kPa, Ca_ppm)
    lai_eff = effective_lai_stanghellini(LAI_val)
    rc = rs_leaf / max(lai_eff, 1e-9)
    gsw = 1.0 / max(0.0224 * rs_leaf, 1e-9)
    return float(rs_leaf), float(rc), float(gsw)

def canopy_photosynthesis_textbook(ppfd_above: float, LAI_val: float, Tleaf_K: float, Ca_ppm: float,
                                   VPD_kPa: float, u_canopy: float):
    """
    Canopy photosynthesis using leaf-scale FvCB biochemistry only.
    Stomatal conductance for ET is computed separately by Jarvis/Stanghellini resistance.
    """
    LAI_eff = max(float(LAI_val), 0.0)
    f_int = beer_lambert_fint(LAI_eff, k=k_sw)
    ppfd_canopy_abs = max(float(ppfd_above), 0.0) * f_int

    leaf_state = solve_leaf_fvcb_no_medlyn(
        ppfd_leaf=(ppfd_canopy_abs / LAI_eff) if LAI_eff > 1e-9 else 0.0,
        Tleaf_K=Tleaf_K,
        Ca_ppm=Ca_ppm,
    )
    rs_leaf, rc_canopy, gsw = stomatal_and_canopy_resistance_from_jarvis(
        rad_wm2=max(float(ppfd_above) / max(PAR_frac * unit_PAR, 1e-9), 0.0),
        Tleaf_K=Tleaf_K,
        VPD_kPa=VPD_kPa,
        Ca_ppm=Ca_ppm,
        LAI_val=LAI_eff,
    ...
    if LAI_eff <= 1e-9:
        return (
            ...
            float(max(Ca_ppm, 1.0)), float(max(Ca_ppm, 1.0)), float(gsw)
        )
    ...
    return (
        ...
        float(leaf_state["Cc"]), float(leaf_state["Cs"]), float(gsw)
    )

def stomatal_and_canopy_resistance(gsw_mol_m2_s: float, LAI_val: float):
    """Compatibility shim; ET path should use Jarvis resistance directly."""
    gsw_eff = max(float(gsw_mol_m2_s), 1e-9)
    rs = 1.0 / (0.0224 * gsw_eff)
    rc = rs / max(effective_lai_stanghellini(LAI_val), 1e-9)
    return float(rs), float(rc)

def canopy_heat_transfer_coeff(T_air_K, u_canopy, Ld=0.09, Lw=0.07, deltaT=0.0):
    """Convective heat-transfer coefficient for canopy sensible/latent exchange [W m-2 K-1]."""
    rho, cp, k_air, mu = air_properties(T_air_K)
    ...
    Gr = (ga * beta * abs(float(deltaT)) * (d ** 3) * (rho ** 2)) / max(mu ** 2, 1e-24)

    Re_term = max(Re, 0.0) ** 1.5
    Gr_term = max(Gr, 0.0) ** 0.75
    Nu = 0.68 * (Re_term + Gr_term) ** (1.0 / 3.0)

    hs = (Nu * k_air) / max(d, 1e-12)
    return float(max(hs, 1e-6))


def compute_Rns(radSolar_out, transGlass, alpha_sw=a_sw_can, k_sw=k_sw, LAI_val: float = 1.0):
    Is = float(radSolar_out) * float(transGlass)
    LAI_eff = max(float(LAI_val), 1e-9)
    f_int = 1.0 - math.exp(-float(k_sw) * LAI_eff)
    Rns = float(alpha_sw) * f_int * Is
    return float(Rns), float(Is), float(f_int)
...
def solve_multilayer_cover_surfaces(
    ...
    Tsky = compute_Tsky_Swinbank_cloud_single(Ti, RH, Te, float(cloud_frac))
...
def compute_qFIR_kW_internal_multilayer(
    ...
    Tsky = compute_Tsky_Swinbank_cloud_single(float(Ti_K), float(np.clip(RH_in_frac, 0.0, 1.0)), float(Te_K), float(cloud_frac))
...
def calculate_covering_properties(screen_on, curtain_on):
    ...
    U_layer = {'PO필름': 5.1, 'AL스크린': 5.5, '보온커튼': 3.1}
...
def dCin_dt_ppm_per_h(Cin_ppm: float, T_in_K: float, T_out_K: float, RH_in: float, RH_out: float, ach: float,
                     ppfd_above: float, LAI: float, Tleaf_K: float, u_canopy: float, C_out_ppm: float = Cout_ppm):
    ...
    (A_can_net, A_leaf_net, _ppfd_can_abs, _f_int, A_can_gross, R_can, J_leaf, Vcmax_T, Cc_est, Cs_est, gsw_leaf) = canopy_photosynthesis_textbook(
        ppfd_above=ppfd_above, LAI_val=LAI, Tleaf_K=Tleaf_K, Ca_ppm=Cin_ppm, VPD_kPa=D_kPa, u_canopy=u_canopy
    )
...
def stanghellini_aerodynamic_resistance(T_air_K: float, Tleaf_K: float, u_canopy: float, leaf_length_m: float) -> float:
    """Stanghellini-type aerodynamic/boundary-layer resistance [s m-1]."""
    l = max(float(leaf_length_m), 1e-6)
    U = max(float(u_canopy), 1e-6)
    dT = abs(float(Tleaf_K) - float(T_air_K))
    return float(max(1174.0 * (l ** 0.5) * ((l * dT + 207.0 * (U ** 2)) ** (-0.25)), 1e-6))


def compute_stanghellini_net_radiation(T_air_K: float, Tleaf_K: float, shortwave_Wm2: float):
    """Stanghellini-style canopy net radiation [W m-2 ground].
    Uses review-paper form: Rn = Rns - Rnl, with Rns = 0.07*Is and
    Rnl = 0.16 * rho * cp * (Ta-Tl) / rR. rR is linearized radiative resistance.
    """
    rho, cp, *_ = air_properties(T_air_K)
    Ta = max(float(T_air_K), 200.0)
    Tl = max(float(Tleaf_K), 200.0)
    Is = max(float(shortwave_Wm2), 0.0)
    rR = float((rho * cp) / max(4.0 * Sigma * (Ta ** 3), 1e-12))
    Rns = 0.07 * Is
    Rnl = 0.16 * rho * cp * ((Ta - Tl) / max(rR, 1e-12))
    Rn = Rns - Rnl
    return float(Rn), float(Rns), float(Rnl), float(rR)
def compute_latent_flux_stanghellini_canopy(T_air_K, RH_in_frac, Rn_Wm2, hs=None, rc=None, area_m2=0.0, LAI_val: float = 1.0, ra_s_m=None, rc_s_m=None):
    RH = float(np.clip(RH_in_frac, 0.0, 1.0))
    ...
    rho, cp, *_ = air_properties(T_air_K)
    if ra_s_m is None:
        hs_eff = max(float(hs), 1e-6)
        ra_leaf = (rho * cp) / hs_eff
    else:
        ra_leaf = max(float(ra_s_m), 1e-6)

    rc_eff = max(float(rc_s_m if rc_s_m is not None else rc), 0.0)
    LAI_eff = max(float(LAI_val), 1e-9)

    numerator = Delta * float(Rn_Wm2) + (2.0 * LAI_eff * rho * cp * VPD_Pa) / max(ra_leaf, 1e-9)
    denominator = Delta + gamma * (1.0 + rc_eff / max(ra_leaf, 1e-9))
...
def compute_Qcrop_sensible_kW(hs, area_m2, deltaT_leaf_air_val, LAI_val: float):
    hs_eff = max(float(hs), 0.0)
    LAI_eff = max(float(LAI_val), 1e-9)
    H_Wm2 = 2.0 * LAI_eff * hs_eff * float(deltaT_leaf_air_val)
    return float(H_Wm2 * float(area_m2) / 1000.0), float(H_Wm2)


def rebuild_to_regular_1min_grid(df: pd.DataFrame, time_col: str = "날짜&시간") -> pd.DataFrame:
    out = df.copy()
...
def run_integrated_TRH_CO2_model(input_xlsx: str) -> pd.DataFrame:
    ...
    df = rebuild_to_regular_1min_grid(df, time_col="날짜&시간")
    ...
    LAI_series = np.full(n, LAI, dtype=float)
    Ld_series = np.full(n, LD, dtype=float)
    Lw_series = np.full(n, LW, dtype=float)

    if n > 1:
        dt_sec = np.full(n, 60.0, dtype=float)
        dt_sec[0] = 60.0
    else:
    ...
    if "배지함수율" in df.columns:
        theta_sub = pd.to_numeric(df["배지함수율"], errors="coerce").interpolate(limit_direction="both").ffill().bfill().to_numpy(float)
        theta_sub = np.where(theta_sub > 1.5, theta_sub / 100.0, theta_sub)
    else:
        raise KeyError("입력 엑셀에 '배지함수율' 컬럼이 없습니다.")

    if "배지온도" in df.columns:
        Ts_sub = pd.to_numeric(df["배지온도"], errors="coerce").interpolate(limit_direction="both").ffill().bfill().to_numpy(float) + 273.15
    else:
        raise KeyError("입력 엑셀에 '배지온도' 컬럼이 없습니다.")
    ...
    screen_on = np.where(rad > 400.0, 1.0, 0.0)

    Tset_cool_K = np.full(n, Tset_cool + 273.15, dtype=float)
    ...
    RHs_soil = np.full(n, np.nan, dtype=float)
    hsoil_Wm2K = np.full(n, np.nan, dtype=float)
    ra_soil_s_m = np.full(n, np.nan, dtype=float)
    rc_s_m = np.zeros(n)
    hs_Wm2K = np.zeros(n)
    ...
    for i in range(n):
        ...
        FCU_on[i] = 1.0 if float(Troom[i]) >= float(Tset_cool_K[i]) else 0.0
        curtain_on[i] = 1.0 if float(Troom[i]) <= (18.0 + 273.15) else 0.0
        ...
        # 2) Otherwise, during cooling operation or rain protection, ACH = ACH_night
        # 3) During daytime, the originally calculated ACH and internal air velocity equation are used as is
        is_night = bool(rad[i] <= 10.0)
        if is_night:
            curr_ach = float(ACH_night)
            curr_u = 0.05
        elif (float(FCU_on[i]) >= 0.5) or (float(rain_on[i]) >= 0.5):
            curr_ach = float(ACH_night)
        ...
        # FCU cooling: Troom >= Cooling load is applied only at 27°C
        # Qcool [kW] = (mHouse*cAir + Chouse) * (Troom - Tset) / 3600
        if float(FCU_on[i]) >= 0.5:
            dT_cool = float(Troom[i] - Tset_cool_K[i])
            cap_kJ_per_K = float(mHouse_i * cAir_kJkgK_i + Chouse)
            qCool = (cap_kJ_per_K * dT_cool) / 3600.0
            qCool = float(max(qCool, 0.0))  # cooling only
        else:
            qCool = 0.0
        ...
        Rns, *_ = compute_Rns(rad[i], transGlass, LAI_val=LAI_t)
        ...
        Rn, Rns_can, Rnl_crop, rR_crop = compute_stanghellini_net_radiation(
            T_air_K=Troom[i], Tleaf_K=Tleaf, shortwave_Wm2=rad[i]
        )
        Rn_Wm2[i] = Rn
        Rnl_crop_Wm2[i] = Rnl_crop

        rad_for_photo = float(rad[i])
        if rad_for_photo < 10.0:
            rad_for_photo = 0.0
        ...
        (
            ...
            ppfd_above=ppfd_above, LAI_val=LAI_t, Tleaf_K=Tleaf, Ca_ppm=Ca_ppm, VPD_kPa=D_kPa, u_canopy=curr_u
        )
        ...
        rs, rc, _gsw_jarvis = stomatal_and_canopy_resistance_from_jarvis(
            rad_wm2=rad[i], Tleaf_K=Tleaf, VPD_kPa=D_kPa, Ca_ppm=Ca_ppm, LAI_val=LAI_t
        )
        rc_s_m[i] = rc
        rs_s_m[i] = rs

        hs = canopy_heat_transfer_coeff(
            Troom[i], curr_u, Ld=Ld_t, Lw=Lw_t, deltaT=abs(deltaT_leaf_air_i)
        )
        hs_Wm2K[i] = hs

        ra_leaf = stanghellini_aerodynamic_resistance(Troom[i], Tleaf, curr_u, Ld_t)
        ...
        theta_i = float(theta_sub[i])
        Ts_i = float(Ts_sub[i])

        psi_m = vg_psi_from_theta(theta_i)
        RHs = kelvin_RH_surface(psi_m, Ts_i)
        RHs_soil[i] = RHs
        ...
        hsoil, _, _, _ = h_soil_from_Nu_flatplate(Troom[i], curr_u, L=soil_depth)
        hsoil_Wm2K[i] = hsoil
        ra_soil = (rho * cp) / max(hsoil, 1e-6)  # s/m
        ra_soil_s_m[i] = ra_soil

        E_soil = (rho / max(ra_soil, 1e-9)) * (Wsurf - Win)
        Wev_raw = E_soil * float(cropArea) * 3600.0
        ...
        Qcrop_kW, _ = compute_Qcrop_sensible_kW(
            hs, cropArea, deltaT_leaf_air_i, LAI_val=LAI_t
        )
        QcropS_kW[i] = Qcrop_kW

        qt[i] = qRad + qRoof + qFloor + qSide + qVent + Qcrop_kW - qLat_net - qFIR - qCool
        ...
        if i < n - 1:
            ...
            Cin_next = Cin_now + dCin_dt_h * dt_step_h
            Cin_ppm[i + 1] = float(np.clip(Cin_next, Cin_min_ppm, Cin_max_ppm))
    ...
    out["FCU_fan"] = FCU_on
    out["Tset_cool"] = Tset_cool_K - 273.15

    out["Wtr_kgph"] = Wtr_kgph
    out["Wev_kgph"] = Wev_kgph
    out["Wev_raw_kgph"] = Wev_raw_kgph
    out["RHs_soil"] = RHs_soil
    out["hsoil_Wm2K"] = hsoil_Wm2K
    out["ra_soil_s_m"] = ra_soil_s_m
    out["rc_s_m"] = rc_s_m
    out["hs_Wm2K"] = hs_Wm2K
    ...
    out["Tfilm_in_C"] = Tfilm_in_K - 273.15
...
def main():
    ...
    ap.add_argument("--input", default="climate/greenhouse_5.xlsx", help="입력 엑셀 경로")
    ap.add_argument("--output", default="", help="출력 엑셀 경로(미지정 시 input 옆에 _with_T_RH_CO2_FvCB.xlsx)")
    args = ap.parse_args()
    ...
    out_path = Path(args.output) if args.output else in_path.with_name(in_path.stem + "_with_T_RH_CO2_FvCB.xlsx")
