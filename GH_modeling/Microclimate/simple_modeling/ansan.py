...
def process_greenhouse_data():
    ...
    df = pd.read_excel("climate/ansan_greenhouse.xlsx")
    ...
    Chouse = 100000  # 온실의 열용량(kJ/k)
    CWhouse = 100000 # 온실의 수분용량(kg)
...
def show_range_8graphs(start_index, end_index, df, Troom, Toutdoor, qt, save_graph=False):
    ...
    plt.figure()
    plt.plot(x_range, sub_Troom - 273.15,   color='black', label='Predicted inside temperature(°C)')
    plt.plot(x_range, sub_Temp_K - 273.15,  color='blue',  label='Measured inside temprature(°C)')
    plt.plot(x_range, sub_Toutdoor - 273.15,color='red',   label='Measured outside temperature(°C)')
    plt.title(f"Predicted & Measured (Inside/Outside) Temperature", fontsize=15)
    plt.xlabel('Hour Index', fontsize=15); plt.ylabel('Temperature(°C)', fontsize=15)
    ...
    if len(troom_c) > 2:
        ...
        y_line = slope * x_line + intercept
        plt.plot(x_line, y_line, color='black', label=f"Regression line\nSlope={slope:.3f}\nR²={r2:.3f}\nRMSE={rmse:.3f}")
        plt.xlabel('Measured inside temperature(°C)', fontsize=15)
        plt.ylabel('Predicted inside temperature(°C)', fontsize=15)
    ...
    plt.figure()
    plt.plot(x_range, sub_RHin,  color='black', label='Predicted inside relative humidity(%)')
    plt.plot(x_range, sub_Humi,  color='blue',  label='Measured inside relative humidity(%)')
    plt.plot(x_range, sub_RHout, color='red',   label='Measured outside relative humidity(%)')
    ...
    if mask.sum() > 2:
        ...
        y_line = slope * x_line + intercept
        plt.plot(x_line, y_line, color='black', label=f"Regression line\nSlope={slope:.3f}\nR²={r2:.3f}\nRMSE={rmse:.3f}")
        plt.xlabel('Measured inside relative humidity(%)', fontsize=15)
        plt.ylabel('Predicted inside relative humidity(%)', fontsize=15)
...
# 해당 데이터는 경기도 안산시의 한 딸기 농가의 온실 환경 데이터로 농림축산식품부 공공데이터에서 가져 옴
