...
def main():
    try:
        ...
        if "~" in user_input:
            ...
            # >>> 추가: 범위용 4패널(온도-회귀-습도-회귀) 한 줄 출력
            sub_df = df.iloc[start_index:end_index + 1].copy()
            sub_Troom = Troom[start_index:end_index + 1]
            sub_Tout  = Toutdoor[start_index:end_index + 1]
            plot_region_panels(
                region_label=f"양주 (범위 {start_dt:%Y-%m-%d %H시} ~ {end_dt:%Y-%m-%d %H시})",  # 필요 시 변경
                df=sub_df,
                Troom=sub_Troom,
                Toutdoor=sub_Tout
            )
            ...
            # >>> 추가: 같은 일자의 4패널(온도-회귀-습도-회귀)을 '맨 밑'에 표시
            idxs = np.where(daily_mask)[0]
            df_day = df.loc[daily_mask].copy()
            Troom_day = Troom[idxs]
            Tout_day  = Toutdoor[idxs]
            plot_region_panels(
                region_label=f"양주 (일자 {selected_date})",
                df=df_day,
                Troom=Troom_day,
                Toutdoor=Tout_day
            )
...
# 해당 데이터는 전라남도 고흥군의 한 딸기 농가의 온실 환경 데이터로 농림축산식품부 공공데이터에서 가져 옴
