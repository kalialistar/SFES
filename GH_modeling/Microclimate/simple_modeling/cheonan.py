...
def process_greenhouse_data():
    ...
    df = pd.read_excel("climate/cheonan_greenhouse.xlsx")
    ...
    Chouse = 600000  # 온실의 열용량(kJ/k)
    CWhouse = 600000 # 온실의 수분용량(kg)
...
def plot_region_panels(region_label, df, Troom, Toutdoor):
    """
    지역별 4패널(온도/온도회귀/습도/습도회귀) 좌→우로 가로 배열
    """
    # --- 데이터 준비
    pred_in_temp_c = np.asarray(Troom) - 273.15
    meas_in_temp_c = df['Temp_K'].to_numpy() - 273.15
    out_temp_c = np.asarray(Toutdoor) - 273.15

    rhin = df['RHin'].to_numpy()
    humi = df['Humi'].to_numpy()
    rhout = df['RHout'].to_numpy()

    x_idx = np.arange(len(df))

    # --- 회귀 계산(온도)
    m_t_mask = ~np.isnan(pred_in_temp_c) & ~np.isnan(meas_in_temp_c)
    if m_t_mask.sum() >= 2:
        x_t = pred_in_temp_c[m_t_mask]
        y_t = meas_in_temp_c[m_t_mask]
        slope_t, intercept_t = np.polyfit(x_t, y_t, 1)
        yhat_t = slope_t * x_t + intercept_t
        r2_t = r2_score(y_t, yhat_t)
        rmse_t = np.sqrt(mean_squared_error(y_t, yhat_t))
    else:
        slope_t = intercept_t = r2_t = rmse_t = np.nan

    # --- 회귀 계산(습도)
    m_h_mask = ~np.isnan(rhin) & ~np.isnan(humi)
    if m_h_mask.sum() >= 2:
        x_h = rhin[m_h_mask]
        y_h = humi[m_h_mask]
        slope_h, intercept_h = np.polyfit(x_h, y_h, 1)
        yhat_h = slope_h * x_h + intercept_h
        r2_h = r2_score(y_h, yhat_h)
        rmse_h = np.sqrt(mean_squared_error(y_h, yhat_h))
    else:
        slope_h = intercept_h = r2_h = rmse_h = np.nan

    fig, axes = plt.subplots(1, 4, figsize=(24, 5))

    # (1) 온도 시계열
    ax = axes[0]
    ax.plot(x_idx, pred_in_temp_c, label='Predicted inside temperature(°C)', color='red')
    ax.plot(x_idx, meas_in_temp_c, label='Measured inside temperature(°C)', color='blue')
    ax.plot(x_idx, out_temp_c, label='Measured outside temperature(°C)', color='black')
    ...
    ax = axes[1]
    if m_t_mask.sum() >= 2:
        ax.scatter(x_t, y_t, alpha=0.5, label='Data', color='green')
        x_line = np.linspace(np.min(x_t), np.max(x_t), 100)
        y_line = slope_t * x_line + intercept_t
        ax.plot(x_line, y_line, color='black', label=f'slope={slope_t:.3f}\nR²={r2_t:.3f}\nRMSE={rmse_t:.3f}')
    ax.set_xlabel('Predicted inside temperature(°C)', fontsize=12)
    ax.set_ylabel('Measured inside temperature(°C)', fontsize=12)
    ax.grid(False)
    ax.legend(fontsize=9)

    # (3) 습도 시계열
    ax = axes[2]
    ax.plot(x_idx, rhin, label='Predicted inside humidity(%)', color='red')
    ax.plot(x_idx, humi, label='Measured inside humidity(%)', color='blue')
    ax.plot(x_idx, rhout, label='Measured outside humidity(%)', color='black')
    ax.set_xlabel('Hour Index', fontsize=12)
    ...
    ax = axes[3]
    if m_h_mask.sum() >= 2:
        ax.scatter(x_h, y_h, alpha=0.5, label='Data', color='green')
        x_line = np.linspace(np.min(x_h), np.max(x_h), 100)
        y_line = slope_h * x_line + intercept_h
        ax.plot(x_line, y_line, color='black', label=f'slope={slope_h:.3f}\nR²={r2_h:.3f}\nRMSE={rmse_h:.3f}')
    ax.set_xlabel('Predicted inside humidity(%)', fontsize=12)
    ax.set_ylabel('Measured inside humidity(%)', fontsize=12)
    ax.grid(False)
    ax.legend(fontsize=9)

    fig.suptitle(f'[ {region_label} ]  온도 → 온도회귀 → 습도 → 습도회귀', fontsize=14, y=1.05)
    plt.tight_layout()
    plt.show()
...
def main():
    try:
        ...
        if "~" in user_input:
            ...
            # 기간 입력 시에도 4패널 그래프 출력
            plot_region_panels(
                f"천안 (기간: {start_dt:%Y-%m-%d} ~ {end_dt:%Y-%m-%d})",
                sub_df,
                Troom[start_index:end_index + 1],
                Toutdoor[start_index:end_index + 1]
            )
            ...
            # --- ADD: 4패널 패널(해당 일자만) 맨 마지막에 붙이기 ---
            daily_idx = np.where(daily_mask)[0]
            daily_start = int(daily_idx[0])
            daily_end = int(daily_idx[-1]) + 1
            plot_region_panels(
                "천안 (해당 일자)",
                df.iloc[daily_start:daily_end].copy(),
                Troom[daily_start:daily_end],
                Toutdoor[daily_start:daily_end]
            )

            daily_df = df.loc[daily_mask].copy()
            export_daily_df = prepare_export_dataframe(daily_df)
            plot_region_panels("천안 (전체)", df, Troom, Toutdoor)
...
# 해당 데이터는 충청남도 천안시의 한 딸기 농가의 온실 환경 데이터로 농림축산식품부 공공데이터에서 가져 옴
