...
# 파장별 광학 계수(Old Plastic 기준)
BAND_PROPS = [
    ("UV",    280.0, 400.0, 0.81, 0.08),
    ("Blue",  400.0, 500.0, 0.86, 0.03),
    ("Green", 500.0, 600.0, 0.87, 0.03),
    ("Red",   600.0, 700.0, 0.89, 0.02),
    ("IR",    700.0, 2500.0, 0.83, 0.09),
]

def build_tau_alpha_arrays(wl_nm: np.ndarray, green_tau_override: float = None):
    tau0 = np.zeros_like(wl_nm, dtype=float)
    alpha0 = np.zeros_like(wl_nm, dtype=float)

    for name, lo, hi, tau, a in BAND_PROPS:
        m = (wl_nm >= lo) & (wl_nm < hi)
        tau0[m] = tau
        alpha0[m] = a
    ...
    if green_tau_override is not None:
        m = (wl_nm >= 500.0) & (wl_nm < 600.0)
        tau0[m] = float(green_tau_override)
...
def compute_case(chosen_dt: pd.Timestamp, orientation: str, green_tau_override: float = None):
    ...
    # 파장별 투과율 및 흡수율 적용
    tau0, alpha0 = build_tau_alpha_arrays(wl, green_tau_override=green_tau_override)

    tau_b = tau0 * Kb
    tau_d = tau0 * Kd

    rho_b = np.maximum(0.0, 1.0 - tau_b - alpha0)
    rho_d = np.maximum(0.0, 1.0 - tau_d - alpha0)
    ...
    E_abs_seg = (alpha0 * E_bn_POA_seg) + (alpha0 * E_d_sky_seg)
    ...
    E_abs_beam_seg = (alpha0 * E_bn_POA_seg)
    E_abs_diff_seg = (alpha0 * E_d_sky_seg)
    ...
    tau_b_eff = (G_trn_beam / Gb) if Gb > 0 else float("nan")
    tau_d_eff = (G_trn_diff / Gd) if Gd > 0 else float("nan")
    rho_b_eff = (G_ref_beam / Gb) if Gb > 0 else float("nan")
    rho_d_eff = (G_ref_diff / Gd) if Gd > 0 else float("nan")
    ...
    meta = {
        ...
        "tau_b": float(tau_b_eff),
        "tau_d": float(tau_d_eff),

        "rho_b": float(rho_b_eff),
        "rho_d": float(rho_d_eff),
...
def save_excel_combined(wl, spectra_by_case, sums_by_case, chosen_dt: pd.Timestamp, filename_suffix: str = "") -> Path:
    Output.mkdir(parents=True, exist_ok=True)
    base = chosen_dt.strftime("%y%mdd%H") if False else chosen_dt.strftime("%y%m%d%H")
    xlsx_path = Output / f"{base}_Optical_results{filename_suffix}.xlsx"
...
if __name__ == "__main__":
    ...
    try:
        ...
        out_path = save_excel_combined(wl_ref, spectra_by_case, sums_by_case, meta_ref["chosen_dt"], filename_suffix="")

        # Green(500-600) 투과율 변경(80/75/70) - 반사율 조정을 위해 사용함
        green_cases = [0.80, 0.75, 0.70]
        spectra_by_case_g = {}
        sums_by_case_g = {}
        wl_ref_g = None
        meta_ref_g = None

        for tau_g in green_cases:
            for surf, ori in cases:
                surf_tag = f"{surf}_G{int(tau_g*100)}"
                (
                    wl, E_ext, E_trn, E_ref, E_abs,
                    E_trn_beam, E_trn_diff,
                    E_ref_beam, E_ref_diff,
                    ...
                    G_crop,
                    meta
                ) = compute_case(qdt, ori, green_tau_override=tau_g)

                if wl_ref_g is None:
                    wl_ref_g = wl
                    meta_ref_g = meta

                spectra_by_case_g[(surf_tag, ori)] = {
                    "ext": E_ext,
        ...
        out_path_green = save_excel_combined(wl_ref_g, spectra_by_case_g, sums_by_case_g, meta_ref["chosen_dt"], filename_suffix="_GreenReflect")
    ...
    print("필름 광학 계수(파장대별)   : (ρ=1-τ-α)")
    for name, lo, hi, tau0, a0 in BAND_PROPS:
        rho0 = 1.0 - float(tau0) - float(a0)
        print(f"  - {name:5s} ({int(lo)}–{int(hi)} nm) : τ={tau0:.3f}, α={a0:.3f}, ρ={rho0:.3f}")
    ...
    print("\n엑셀 저장(통합) :", out_path, "\n")
    print("엑셀 저장(녹색광 반사 케이스 80/75/70) :", out_path_green, "\n")
